...
class etat:
    ...
    def acc(self):
        ...
        acceptables=[etat(coords=ac, prec=self, arrivee=self.arrivee) for ac in accessibles
                      if ((1<=ac[0]<=labyrinthe.largeur) and
                      (1<=ac[1]<=labyrinthe.hauteur) and ac not in [mur.coords for
                      mur in labyrinthe.murs])]
        return acceptables
...
class lab:
    ...
    murs=[etat(coords=(2, 5)), etat(coords=(3, 2)), etat(coords=(3, 3)), etat(coords=(3, 5)), etat(coords=(3, 6)), etat(coords=(3, 7)), etat(coords=(3, 8)), etat(coords=(3, 9)),
          etat(coords=(4, 3)), etat(coords=(5, 3)), etat(coords=(5, 5)), etat(coords=(5, 6)), etat(coords=(6, 3)), etat(coords=(6, 5)), etat(coords=(6, 6)), etat(coords=(6, 8)),
          etat(coords=(7, 8)), etat(coords=(8, 2)), etat(coords=(8, 3)), etat(coords=(8, 4)), etat(coords=(8, 8)), etat(coords=(9, 4)), etat(coords=(9, 8)), etat(coords=(10, 4))]

def astar(etat):
   open_list = pile()
   closed_list = set()
   
   open_list.fifo(commence)
   
   while open_list.pile:
    current_state = open_list.pile.pop(0)  
    closed_list.add(current_state.coords)  
    if current_state.coords == but.coords:
        path = []
        while current_state:
...
pile=pile()
print(initial.acc())
print(points_cardinaux().deplacements())
